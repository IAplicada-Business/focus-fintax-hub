...
export function ClienteOperacaoEditor({
  ...
  const etapas = useMemo(() => {
    const configuradas = [...(slaConfigQ.data ?? [])]
      .sort((a, b) => a.ordem - b.ordem)
      .filter(
        (item) =>
          item.ativo || item.estagio === cliente.estagio_esteira,
      );
    return configuradas.filter((item) => isEstagioEsteira(item.estagio));
    ...
    <div className="space-y-3 rounded-lg border p-3">
      <div>
        ...
        <p className="text-[10px] text-muted-foreground">
          Status, responsável e etapa atual da empresa.
        </p>
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-[10px] text-muted-foreground">
          Ao mudar a etapa, o histórico é auditado e o SLA recomeça.
        </p>
      </div>
