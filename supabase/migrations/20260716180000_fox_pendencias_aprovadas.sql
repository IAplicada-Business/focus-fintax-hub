...
--  1) enum tributo += ICMS  (ANTES do BEGIN — PG não usa valor novo na mesma txn em alguns casos)
--  2) security_invoker nas views (Reporto não vaza pra comercial/cliente)
...
-- 1) ICMS como tributo próprio (fora do BEGIN)
DO $$ BEGIN
...
-- 2) Views com security_invoker (respeitam RLS do caller)
DO $$
BEGIN
...
-- 4) Promove ICMS que entrou como 'outros'
...
-- 5) Backfill tese_origem_id — FIFO (INSUMOS → SUBVENCAO → demais do cálculo)
